...
# --------------------------------------------
# Run from JupyterLab or using the script '_run_all_calcFWI_CMIP6.sh' for server?
run_on_server = False
...
for item in cfg.list_sorted_common:
    ...
    if cfg.option_full_outputs:
        name_file = (
            "fwi_day_" + esm + "_" + xp + "_" + member + "_" + grid + "_full-outputs.nc"
        )
    else:
    ...
    if function_check_run_file(name_file, cfg):
        ...
        if (run_on_server == False) or (
            ...
            full_dates = {
                2014: type_time(
                    2014, 12, 31 - 1 * (type_time == cft.Datetime360Day), 12, 0, 0, 0
                ),
                2039: type_time(
                    2039, 12, 31 - 1 * (type_time == cft.Datetime360Day), 12, 0, 0, 0
                ),
                2040: type_time(
                    2040, 12, 31 - 1 * (type_time == cft.Datetime360Day), 12, 0, 0, 0
                ),
            }

            # initialization
            vars_calc = (
                init_prev_values(DATA, cfg)
                if xp in ["historical"]
                else func_init_from_scen(item, full_dates, run_FWI.time.values[0], cfg)
            )
            ...
            for (k_time, val_time) in enumerate(run_FWI.time.values):
                ...
                former_vars_calc = {
                    var: np.copy(vars_calc[var]) for var in cfg.vars_transmit_timesteps
                }
                ...
                if np.any(
                    np.isnan(
                        vars_calc["fwi"][
                            vars_calc["ind_calc_FWI"][0], vars_calc["ind_calc_FWI"][1]
                        ]
                    )
                ):
                ...
                if (xp in cfg.list_xp_init) and (val_time in full_dates.values()):
                    for key in cfg.vars_transmit_timesteps:
                        dico_mem_PREV[key + "_" + str(val_time.year)] = np.copy(
                            vars_calc[key]
                        )
            ...
            if xp in cfg.list_xp_init:
                ...
                for key in dico_mem_PREV.keys():
                    if dico_mem_PREV[key].ndim == 3:
                        run_FWI[key] = xr.DataArray(
                            dico_mem_PREV[key], dims=("days_wDC", "lat", "lon")
                        )
                    else:
                        run_FWI[key] = xr.DataArray(
                            dico_mem_PREV[key], dims=("lat", "lon")
                        )
                    run_FWI[key].attrs[
                        "info"
                    ] = "Simply saving this variable to initialize the scenarios using historical values"
                    run_FWI[key].attrs["time_values"] = str(
                        full_dates[int(str.split(key, "_")[-1])]
                    )
                # reset attributes
            ...
            print(
                "Computation time: "
                + str(np.round((ti_end - ti_start) / 60, 1))
                + "min"
            )
            ...
            if ("_".join(item) in cfg.runs_exceptions_okay.keys()) and (
                ...
                in cfg.runs_exceptions_okay["_".join(item)]
            ):
            ...
            with warnings.catch_warnings():
                ...
                run_FWI.to_netcdf(
                    cfg.path_saveFWI + "/" + name_file,
                    encoding={var: {"zlib": True} for var in run_FWI.variables},
                )
