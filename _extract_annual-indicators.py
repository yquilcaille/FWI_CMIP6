...
for indic in list_indics:
    os.makedirs(
        os.path.join(path_FWInative_annual, indic, new_time_res, "g025"), exist_ok=True
    )
# --------------------------------------------
# --------------------------------------------
...
for name_item in dico_items.keys():
    ...
    if np.any(
        list(test_NeedRun.values())
    ):  # if so, adding them all: required because of historical period as reference for scenarios
        counter_missing += len(
            dico_items[name_item]
        )  # counting how many scenarios to treat here
...
type_variables = "-".join(
    str.split(path_FWInative_day[path_FWInative_day.find("hurs") :], "_")[: 1 + 1]
)
with open(
    "available_files/" + "exceptions_" + type_variables + ".csv", newline=""
) as csvfile:
...
for name_item in list_tmp:
    ...
    name_files_W = [
        "_".join([var, current_time_res, esm, scen, memb, grid]) + ".nc"
        for scen in dico_items[name_item]
    ]
    ...
    with warnings.catch_warnings():
        ...
        DATA = xr.open_mfdataset(
            files_to_get, preprocess=add_xp_dim, combine="nested", concat_dim="xp"
        )  # , chunks={'time':1}

    # calculating annual indicators
    to_save = calc_indicators(
        DATA, ref_period, indics=list_indics, method_fwixd=method_fwixd
    )

    # saving each xp in different files
    for scen in dico_items[name_item]:
        print(
            "Calculating and saving " + esm + ", " + memb + " on " + scen
        )  # +' for '+indic )
        for indic in list_indics:
            name_tmp = "_".join([indic, new_time_res, esm, scen, memb, grid]) + ".nc"
            path_save = os.path.join(
                path_FWInative_annual, indic, new_time_res, "g025", name_tmp
            )
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                func_save_xp(
