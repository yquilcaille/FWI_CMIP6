...
list_files = [os.path.join( path_fwi_cmip6_native, file_W) for file_W in os.listdir(path_fwi_cmip6_native) if 'full-outputs' not in file_W ]
...
to_remove = {'fwi_day_GFDL-CM4_historical_r1i1p1f1':['gr1','gr2'], \
             'fwi_day_GFDL-CM4_ssp585_r1i1p1f1':['gr1','gr2'], \
             'fwi_day_GFDL-CM4_ssp245_r1i1p1f1':['gr1','gr2'] }
for file_remove in to_remove:
    test = {grid:os.sep.join( [path_fwi_cmip6_native, file_remove+'_'+grid+'.nc'] ) in list_files for grid in to_remove[file_remove]}
    if sum(test.values()) > 1:
        run_to_rm = False
        for grid in to_remove[file_remove]:
            if test[grid] and run_to_rm:# other grid present in files, assumed not the best, removing it
                pth = os.sep.join( [path_fwi_cmip6_native, file_remove+'_'+grid+'.nc'] )
...
for item in dic_full:
    if len(dic_full[item]) > 1:
        print("Warning: several runs still share the same ESM, experiment and ensemble member, but differ by their grid. Please choose the initial grid for regridding. Happens in    "+item)
#=====================================================================
#=====================================================================
...
args = type('cfg_check', (object,), {'overwrite':False, 'log_level':20, 'log_file':None, 'debug':False})()
...
def get_path_out_regrid( files ):
    ...
    save_path = sf.get_ng_path(varn=metadata['varn'],time_res=time_res,SAVE_PATH=SAVE_PATH)
...
for files in list_files:
    ...
    cfg = type('cfg_check', (object,), {'path_saveFWI':path_tmp[0], 'option_overwrite':option_overwrite, 'overwrite_before':overwrite_before})()
        ...
        if (run_on_server==False)  or  (counter_missing in range(subindex_csl*runs_per_process,(subindex_csl+1)*runs_per_process)):
            ...
            try:
                filename, updated = cf.process_native_grid(
                    [files], time_res, git_info=git_info, overwrite=args.overwrite, debug=args.debug, SAVE_PATH=SAVE_PATH)
                ...
                if not gf.delete_corrupt_files(source):  # check/delete corrupt source
                    ...
                    logger.error(errmsg)
print('****************************************************************************************************')
print('*  FOR MEMORY REASONS, DO NOT FORGET TO DELETE THE INTERMEDIARY REPOSITORY AND ALL FILES INSIDE!!  *')
print('****************************************************************************************************')
#--------------------------------------------
#--------------------------------------------
