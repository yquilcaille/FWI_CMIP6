...
def FFMC(TEMP,RH,WIND,RAIN,FFMCPrev):
    ...
    mr = mo[ind_RG05] + 42.5 * rf * np.exp(-100.0 / (251.0 - mo[ind_RG05])) * (1.0-np.exp(-6.93 / rf))
    # case of m0 > 150: need to add a single term
    ind_moG150 = np.where(mo[ind_RG05] > 150)
    mr[ind_moG150] += 0.0015 * pow(mo[ind_RG05][ind_moG150] - 150.0, 2) * pow(rf[ind_moG150], .5)
    # limiting mr to 250
    ...
    # Preparing threshold
    ed = 0.942 * pow(RH, 0.679) + 11.0 * np.exp((RH - 100.0) / 10.0) + 0.18 * (21.1 - TEMP) * (1.0 - np.exp(-0.115 * RH))
        
    # Case mo > ed
    ind_moGed = np.where( mo > ed )
    ko = 0.424 * (1.0 - pow(RH[ind_moGed] / 100.0, 1.7)) + 0.0694 * pow(WIND[ind_moGed], .5) * (1.0 - pow(RH[ind_moGed] / 100.0, 8))
    ...
    ew = 0.618 * pow(RH[ind_moLed],0.753) + 10.0 * np.exp((RH[ind_moLed] - 100.0) / 10.0) + 0.18 * (21.1 - TEMP[ind_moLed]) * (1.0 - np.exp(-0.115 * RH[ind_moLed]))
    #   Subcase m0 < ew
    ind_moLew = np.where( mo[ind_moLed] < ew )
    k1 = 0.424 * (1.0 - pow((100.0 - RH[ind_moLed][ind_moLew]) / 100.0, 1.7)) + 0.0694 * pow(WIND[ind_moLed][ind_moLew], .5) * (1.0 - pow((100.0 - RH[ind_moLed][ind_moLew]) / 100.0, 8))
    kw = k1 * 0.581 * np.exp(0.0365 * TEMP[ind_moLed][ind_moLew])
    tmp[ind_moLew] = ew[ind_moLew] - (ew[ind_moLew] - mo[ind_moLed][ind_moLew]) * pow(10.0, -kw)
    #   Subcase m0 >= ew
...
def DMC(TEMP,RH,RAIN,DMCPrev,LAT,numb_day,MONTH,cfg):
    ...
    k[ind_TEMPG11] = 1.894 * (TEMP[ind_TEMPG11] + 1.1) * (100.0 - RH[ind_TEMPG11]) * d1[ind_TEMPG11] * 0.000001
...
def BUI(DMC,DC):
    ...
    ind_dmc0dc0 = np.where( np.isclose(DMC[ind_dmcL04dc],0) & np.isclose(DC[ind_dmcL04dc],0) )
    tmp[ind_dmc0dc0] = 0
    ind_dmcN0dcN0 = np.where( ~np.isclose(DMC[ind_dmcL04dc],0) | ~np.isclose(DC[ind_dmcL04dc],0) )
    tmp[ind_dmcN0dcN0] = 0.8 * DMC[ind_dmcL04dc][ind_dmcN0dcN0] * DC[ind_dmcL04dc][ind_dmcN0dcN0] / (DMC[ind_dmcL04dc][ind_dmcN0dcN0] + 0.4 * DC[ind_dmcL04dc][ind_dmcN0dcN0])
    U[ind_dmcL04dc] = tmp
    ...
    tmp = U[ind_dmcG04dc]
    # handling the cases of DMC and DC being 0
    ind_dmc0dc0 = np.where( np.isclose(DMC[ind_dmcG04dc],0) & np.isclose(DC[ind_dmcG04dc],0) )
    tmp[ind_dmc0dc0] = 0
    ind_dmcN0dcN0 = np.where( ~np.isclose(DMC[ind_dmcG04dc],0) | ~np.isclose(DC[ind_dmcG04dc],0) )
    tmp[ind_dmcN0dcN0] = DMC[ind_dmcG04dc][ind_dmcN0dcN0] - (1.0 - 0.8 * DC[ind_dmcG04dc][ind_dmcN0dcN0] / (DMC[ind_dmcG04dc][ind_dmcN0dcN0] + 0.4 * DC[ind_dmcG04dc][ind_dmcN0dcN0])) * (0.92 + pow(0.0114 * DMC[ind_dmcG04dc][ind_dmcN0dcN0], 1.7))
    U[ind_dmcG04dc] = tmp
...
def DryingFactor(Latitude, Month, cfg):
    if len(np.array(Month).shape) > 0:
        raise Exception('This code has been rewritten so that only the month must be a scalar... dont know what to do if there are multiple values.')
...
def test_FireSeason(TEMP_wDC, SeasonActive, threshold_start = 12, threshold_end = 5):
    ...
    test_start = np.where(np.all( (TEMP_wDC[2:,ind_SeasonNotActive[0],ind_SeasonNotActive[1]] > threshold_start), axis=0 ))
    ...
    test_end = np.where(np.all( (TEMP_wDC[:2+1,ind_SeasonActive[0],ind_SeasonActive[1]] < threshold_end), axis=0 ))
...
def DC_with_overwintering( TEMP_wDC, FormerSeasonActive, rw, dcf, CounterSeasonActive, dcPREV, TEMP, RAIN, LAT, MONTH, cfg ):
    '''
    ...
    dc = np.nan * np.ones( CurrentSeasonActive.shape ) # values at this current timestep: either we calculate it or it will be nan because not in the fire season
    ...
    dc[ind_TT] = DC(TEMP[ind_TT], RAIN[ind_TT], dcPREV[ind_TT], LAT[ind_TT], MONTH, cfg) # (case 1.1)
    ...
    dc_tmp[ind_CSA0] = DC(TEMP[ind_FT][ind_CSA0], RAIN[ind_FT][ind_CSA0], dcPREV[ind_FT][ind_CSA0], LAT[ind_FT][ind_CSA0], MONTH, cfg) # (case 2.1.1)
    # For other seasons, values for dcf and rw are correctly calculated.
    ind_CSAG0 = np.where( CounterSeasonActive[ind_FT] > 0 )    
    dc_tmp[ind_CSAG0] = wDC( dcf[ind_FT][ind_CSAG0], rw[ind_FT][ind_CSAG0], a=0.75, b=0.75 ) # (case 2.1.2)
    # affecting to dc
    dc[ind_FT] = dc_tmp
    rw[ind_FT] = 0
    dcf[ind_FT] = np.nan
    CounterSeasonActive[ind_FT] = CounterSeasonActive[ind_FT] + 1 # next season can be overwintered
    ...
    dc_tmp[ind_CSA0] = DC(TEMP[ind_FF][ind_CSA0], RAIN[ind_FF][ind_CSA0], dcPREV[ind_FF][ind_CSA0], LAT[ind_FF][ind_CSA0], MONTH, cfg) # (case 2.2.1)
    dc[ind_FF] = dc_tmp
    ...
    ind_calc_FWI = np.where( CurrentSeasonActive  |  ((CurrentSeasonActive==False) & (CounterSeasonActive==0)) )
    
    return {'dc':dc, 'SeasonActive':CurrentSeasonActive, 'rw':rw, 'DCf':dcf, 'CounterSeasonActive':CounterSeasonActive, 'ind_calc_FWI':ind_calc_FWI}
...
def calcFWI( vars_calc, cfg ):
    ...
    if cfg.adjust_overwinterDC == 'wDC':
        out = DC_with_overwintering(vars_calc['TEMP_wDC'], vars_calc['SeasonActive'], vars_calc['rw'], vars_calc['DCf'], vars_calc['CounterSeasonActive'],\
                                    vars_calc['dcPREV'], vars_calc['TEMP'], vars_calc['RAIN'], vars_calc['LAT'], vars_calc['MONTH'], cfg )
        for var in ['dc', 'SeasonActive', 'rw', 'DCf', 'CounterSeasonActive', 'ind_calc_FWI']:
            vars_calc[var] = np.copy( out[var] )
    else:
        vars_calc['dc'] = DC(vars_calc['TEMP'], vars_calc['RAIN'], vars_calc['dcPREV'], vars_calc['LAT'], vars_calc['MONTH'], cfg)
        # without overwintering, will always calculates FWI everywhere
        shp_lat, shp_lon = vars_calc['LAT'].shape
        vars_calc['ind_calc_FWI'] = ( np.repeat( np.arange(shp_lat), shp_lon), np.repeat( np.arange(shp_lon)[np.newaxis,:], shp_lat, axis=0).flatten() )
        
    # Calculating the 2 other codes:
    vars_calc['ffmc'] = FFMC(vars_calc['TEMP'], vars_calc['RH'], vars_calc['WIND'], vars_calc['RAIN'], vars_calc['ffmcPREV'])
    vars_calc['dmc'] = DMC(vars_calc['TEMP'], vars_calc['RH'], vars_calc['RAIN'], vars_calc['dmcPREV'], vars_calc['LAT'], vars_calc['NUMB_DAY'], vars_calc['MONTH'], cfg)
