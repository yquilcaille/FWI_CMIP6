...
def calc_indicators(
    data,
    ref_period,
    indics=["fwixx", "fwils", "fwixd", "fwisa"],
    ...
    if type_time in [np.datetime64]:
        ...
        day_end = type_time(
            ref_period[1], 12, 31 - 1 * (type_time == cft.Datetime360Day), 12, 0, 0, 0
        )
    ...
    with warnings.catch_warnings():
        ...
        if "fwixx" in indics:  # annual maxima
            ...
            OUT["fwixx"].attrs[
                "method_fwixx"
            ] = "Annual maximum over each year of the FWI"
        ...
        if "fwils" in indics:  # length fire weather season
            ...
            OUT["fwils"].attrs[
                "method_fwils"
            ] = "Number of days for which the FWI is above the mean of minimum and maximum of the FWI over the reference period for this single ensemble member."
        ...
        if "fwixd" in indics:  # nb of days extreme fire weather
            OUT["fwixd"] = calc_indic_fwixd(
                data["fwi"], day_start, day_end, ref_xp, method=method_fwixd
            )
            ...
            if "category" in method_fwixd:
                OUT["fwixd"].attrs["method_fwixd"] = (
                    "Counted days above a threshold defined as the "
                    + str(dico_bounds_dangers[lvl][0])
                    + " - "
                    + str(dico_bounds_dangers[lvl][1])
                    + " fire risk in the Canadian Fire Weather Index categories"
                )
        ...
        if "fwisa" in indics:  # seasonal average
            ...
            OUT["fwisa"].attrs[
                "long_name"
            ] = "Seasonal Average Of The Fire Weather Index"
    ...
    if (
        data["fwi"].attrs["description"]
        != "Very low danger: FWI < 5.2; Low danger: 5.2 <= FWI < 11.2; Moderate danger: 11.2 <= FWI < 21.3; High danger: 21.3 <= FWI < 38.0; Very high danger: 38.0 <= FWI < 50; Extreme danger: 50 < FWI"
    ):
        for indic in indics:
...
def calc_indic_fwixd(dat, day_start, day_end, ref_xp, method):
    ...
    if "category" in method:
        ...
        local_threshold = (
            xr.where(np.isnan(tmp) == False, tmp, 0)
            .quantile(q=lvl / 100, dim=("xp", "time"))
            .drop("quantile")
        )  # need to identify NaN because of overwintering
...
def calc_indic_fwisa(dat):
    ...
    """
    rollmean = (
        xr.where(np.isnan(dat) == False, dat, 0).rolling(time=90, center=True).mean()
    )  # need to identify NaN because of overwintering
...
def xr_interpolate_nan_grid(data, method="linear"):
    ...
    if "time" in data.dims:
        for t in data.time:
            out.loc[{"time": t}] = np_interpolate_nan_grid(
                array=data.sel(time=t).values,
                lat_mesh=la_mesh,
                lon_mesh=lo_mesh,
                method=method,
            )
    else:
        out[...] = np_interpolate_nan_grid(
            array=data.values, lat_mesh=la_mesh, lon_mesh=lo_mesh, method=method
        )
...
def func_save_xp(full_data, xp, indic, path_save, attrs_out, check_NaN=True):
    ...
    if type_time in [np.datetime64]:
        new_time = [
            np.datetime64(str(yr) + "-07-01T06:00:00")
            for yr in full_data.time.dt.year.values
        ]
    else:
        new_time = [
            type_time(yr, 7, 1, 6, 0, 0, 0) for yr in full_data.time.dt.year.values
        ]
    full_data.coords["time"] = new_time

    # preparing the correct period
    period = {
        **{"historical": (1850, 2014), "ssp534-over": (2040,)},
        **{
            ss: (2015,)
            for ss in [
                "ssp119",
    ...
    if len(period) == 2:
        data_save = (
            full_data.sel(xp=xp)
            .isel(time=slice(period[0] - y0, period[1] - y0 + 1))
            .drop("xp")
        )
    else:
        ...
        data_save = (
            full_data.sel(xp=xp)
            .isel(time=slice(period[0] - y0, end - y0 + 1))
            .drop("xp")
        )
        ...
        if var not in data_save.coords:
            if np.any(np.isnan(data_save[var])):
                if (check_NaN == False) and (var in [indic]):
                    ...
                    data_save[var] = xr_interpolate_nan_grid(
                        data_save[var], method="linear"
                    )
    ...
    data_save.attrs[
        "experiment_id"
    ] = xp  # dont forget it, we are saving each experiment separately, while the esm and member attributes are unchanged.
    ...
    if warning_nan:
        data_save[indic].attrs[
            "handling_NaN"
        ] = "Some points had NaN values, they have been filled using 2D linear interpolation."

    # actually saving
    data_save.to_netcdf(
        path_save, encoding={var: {"zlib": True} for var in data_save.variables}
    )
