...
class configuration_FWI_CMIP6:
    ...
    #--------------------
    # INITIALIZATION    
    def __init__(self, type_variables, adjust_DryingFactor, adjust_DayLength, adjust_overwinterDC, limits_on_data, path_cmip6, path_save, overwrite_path_saveFWI, option_overwrite, overwrite_before, option_load_available, option_calc_size, option_full_outputs):
    ...
    def prepare_variables( self ):
        ...
        if len(vars_) != 2:
            ...
            raise Exception('First one in "type_variables" should be in hurs, hursmin, hursmax.')

        elif vars_[1] not in ['tas', 'tasmin', 'tasmax']:
            raise Exception('First one in "type_variables" should be in tas, tasmin or tasmax.')
        ...
        if self.option_load_available:
            ...
            for file_W in os.listdir( 'available_files'):
                if self.type_variables+'.csv' in file_W:
                    with open('available_files'+os.sep+file_W, newline='') as csvfile:
                        ...
                        for row in read:
                            if len(row)==1:
                                ...
                                tmp.append(row)
                        self.COMMON[file_W[:-len('_'+self.type_variables+'.csv')]] = tmp
            ...
            for key in self.COMMON.keys():
                with open('available_files'+os.sep+key+'_'+self.type_variables+'.csv', 'w', newline='') as csvfile:
    ...
    def prepare_exceptions( self ):
        ...
        with open('available_files'+os.sep+'exceptions_'+self.type_variables+'.csv', newline='') as csvfile:
            read = csv.reader(csvfile, delimiter=',')
        ...
        with open('available_files'+os.sep+'exclusions_'+self.type_variables+'.csv', newline='') as csvfile:
            read = csv.reader(csvfile, delimiter=',')
            self.runs_to_avoid = []
            for row in read:
    ...
    def prepare_repository( self ):
        if self.overwrite_path_saveFWI is None:
            ...
            name_folder_adjusts = '_'.join( ['Drying-'+self.adjust_DryingFactor, 'Day-'+self.adjust_DayLength, 'Owinter-'+self.adjust_overwinterDC] )
            self.path_saveFWI = os.path.join( self.path_save, name_folder_variables, name_folder_adjusts, 'intermediary' )
    ...
    def func_prepare_files( self ):
        ...
        if self.limits_on_data['xps'] is None:
            self.list_xps = ['historical', 'ssp119', 'ssp126', 'ssp245', 'ssp370', 'ssp434', 'ssp460', 'ssp585', 'ssp534-over']
        else:
        ...
        for item in self.runs_to_avoid:
            ...
            if item[1] in ['historical']:
                for xp in self.COMMON['xps']:
                    if ([item[0], xp, item[2], item[3]] in self.COMMON['runs']) and (xp not in ['historical']):
        ...
        prepared_order_xp = ['historical', 'ssp119', 'ssp126', 'ssp245', 'ssp370', 'ssp434', 'ssp460', 'ssp585', 'ssp534-over']
        ...
        self.vars_transmit_timesteps = ['ffmcPREV', 'dcPREV', 'dmcPREV'] + (self.adjust_overwinterDC == 'wDC') * ['TEMP_wDC', 'SeasonActive', 'DCf', 'rw', 'CounterSeasonActive']
                        ...
                        for member in list_members:
                            ...
                            for tmp in os.listdir( path_grids ):
                                if len( os.listdir(os.path.join(path_grids,tmp)) ) > 0:
                                    self.dico_available_runs[xp][var][esm][member].append( tmp )
                            ...
                            if self.option_calc_size:
                                for grid in self.dico_available_runs[xp][var][esm][member]:
                                    ...
                                    for file_W in list_files:
                                        if '.nc' in  file_W:
                                            counter_before += os.path.getsize( os.path.join(path_files, file_W) )
        if self.option_calc_size:
            print( 'initial size of ALL the data: '+str(np.round(counter_before / (1024)**4, 1))+'Tb' )
                ...
                for member in list_members_all:
                    for grid in list_grids_all:
                        ...
                        if np.all( avail_all ):
                            ...
                            if xp in ['historical']:
                            ...
                            elif xp in ['ssp119', 'ssp126', 'ssp245', 'ssp370', 'ssp434', 'ssp460', 'ssp585', 'ssp534-over']:
                                avail_all_hist = self.func_test_vars('historical', esm, member, grid)
                                ...
                                raise Exception('Unprepared experience, need to decide about its continuity to a historical run for the initialization of the FWI.')
        print("Number of ESMs used: "+str(len(list_esms_common))+" (initially in data: "+str(len(list_esms_all))+")")
        print("Number of members used: "+str(len(list_members_common))+" (initially in data: "+str(len(list_members_all))+")")
        print("Number of grids used: "+str(len(list_grids_common))+" (initially in data: "+str(len(list_grids_all))+")")
        ...
        if self.option_calc_size:
            ...
            for item in list_runs_common:
                ...
                for var in list_vars:
                    path_files = os.path.join( self.path_cmip6, xp, 'day', var, esm, member, grid )
                    list_files = os.listdir( path_files )
                    for file_W in list_files:
                        if '.nc' in  file_W:
                            counter_after += os.path.getsize( os.path.join(path_files, file_W) )
            print( 'Size of the data that will actually be used: '+str(np.round(counter_after / (1024)**4, 1))+'Tb' )

        return {'runs':list_runs_common, 'esms':list_esms_common, 'members':list_members_common, 'grids':list_grids_common, 'xps':self.list_xps}
    #--------------------
...
def convert_rain(rain_cmip6, k_time):
    ...
    if rain_cmip6.units not in ['kg m-2 s-1']:
        ...
        return rain_cmip6.isel(time=k_time).values * 24 * 3600 / 1.
...
def prepare_variables_FWI( former_calc, DATA, k_time, cfg ):
    ...
    calc['NUMB_DAY'] = val_time.dayofyr # TO DO: adapt by factor depending on calendar for number of days in year --> 365
    ...
    if (cfg.adjust_overwinterDC == 'wDC'):
        if 'tasmax' not in DATA:
            raise Exception('The detection of the fire season is meant to be used with the daily maximum temperature (tasmax).')
        else:
            # adding 2 days before, the current day and the next 2 days of tasmax if need to overwinter DC
            if (former_calc['CounterSeasonActive'] is None)  or  (former_calc['CounterSeasonActive'].ndim == 0):# corresponds to (k_time == 0) and xp in ['historical']:
                # Approximation: initialization of this variable, with 3 times the first timestep
                calc['TEMP_wDC'] = convert_temp( DATA['tasmax']['tasmax'] , [0,0,0,1,2] )
            else:
                # adding 2 days before, the current day and the next 2 days of tasmax if need to overwinter DC
                # Approximation: last timesteps of the run are repeated instead of using scenarios.
                temp_new = convert_temp( DATA['tasmax']['tasmax'] , np.min( [DATA['tasmax'].time.size-1, k_time+2] ) )
                calc['TEMP_wDC'] = np.vstack([former_calc['TEMP_wDC'][1:,...], temp_new[np.newaxis,...]])
            ...
            if (former_calc['CounterSeasonActive'] is None)  or  (former_calc['CounterSeasonActive'].ndim == 0):# corresponds to (k_time == 0) and xp in ['historical']:
                calc['SeasonActive'] = False * np.empty( calc['LAT'].shape, dtype=bool ) # SeasonActive: Boolean to check whether the fire season is active or not
                calc['DCf'] = np.nan * np.ones( calc['LAT'].shape ) # DCf: Final fall DC value
                calc['rw'] = np.zeros( calc['LAT'].shape ) # rw: Precipitations accumulated over winter
                calc['CounterSeasonActive'] = np.zeros( calc['LAT'].shape ) # CounterSeasonActive: counts how many fire seasons there has been. Required for the case of the first active fire season.
            else:
        ...
        raise Exception('Wrong name for adjust_overwinterDC, only wDC (=True) and original (=False) are possible values.')
    ...
    for var in ['WIND', 'RAIN', 'RH'] + (cfg.adjust_overwinterDC != 'wDC')*['TEMP']:
        ...
        fct_convert = {'WIND':convert_wind, 'RAIN':convert_rain, 'TEMP':convert_temp, 'RH':convert_rh}[var]
        ...
        if var == 'WIND':
            ...
            vars_tmp = [var for var in DATA.keys() if var in ['tas', 'tasmax', 'tasmin']]
            if len( vars_tmp ) > 1:
                raise Exception('It seems that several variables could be interpreted as temperature.')
            elif len( vars_tmp ) == 0:
                raise Exception('It seems that no variables could be interpreted as temperature.')
            else:
                var_cmip6 = vars_tmp[0]
        elif var == 'RH':
            vars_tmp = [var for var in DATA.keys() if var in ['hurs', 'hursmax', 'hursmin']]
            if len( vars_tmp ) > 1:
                raise Exception('It seems that several variables could be interpreted as relative humidity.')
            elif len( vars_tmp ) == 0:
                raise Exception('It seems that no variables could be interpreted as relative humidity.')
            else:
                var_cmip6 = vars_tmp[0]
...
def function_check_run_file( name_file, cfg ):
    ...
    if os.path.isfile(cfg.path_saveFWI+os.sep+name_file)==False:
        ...
        if cfg.option_overwrite==False:
            ...
            t_f = np.datetime64(dt.date.fromtimestamp(os.path.getmtime(cfg.path_saveFWI+os.sep+name_file)))
...
def func_init_from_scen( item, full_dates, date0, cfg ):
    ...
    if ('_'.join(item) in cfg.runs_exceptions_okay.keys())  and  ('correction_ssp534-over_ssp585-2039' in cfg.runs_exceptions_okay['_'.join(item)]):
        xp_init = 'ssp585' # corresponding warning already embedded in the dataset run_FWI.
        date_init = full_dates[2039]
    elif ('_'.join(item) in cfg.runs_exceptions_okay.keys())  and  ('correction_ssp534-over_ssp585-2040' in cfg.runs_exceptions_okay['_'.join(item)]):
        xp_init = 'ssp585' # corresponding warning already embedded in the dataset run_FWI.
    ...
    if cfg.option_full_outputs:
        init_file = 'fwi_day_'+esm+'_'+xp_init+'_'+member+'_'+grid+'_full-outputs.nc'
    else:
    ...
    if (date0 - init.time.sel(time=date_init).values).days == 1:
        for kk in cfg.vars_transmit_timesteps:
            ...
            if init[kk+'_'+str(date_init.year)].attrs['time_values'] != str(date_init):
                raise Exception('The initialization seems not to have been saved at the correct date.')
            else:
...
def adhoc_concat2040(item, FWI, cfg):
    ...
    esm,xp,member,grid = item
    if cfg.option_full_outputs:
        init_file = 'fwi_day_'+esm+'_'+'ssp585'+'_'+member+'_'+grid+'_full-outputs.nc'
    else:
        init_file = 'fwi_day_'+esm+'_'+'ssp585'+'_'+member+'_'+grid+'.nc'
    init = xr.open_dataset( cfg.path_saveFWI+os.sep+init_file, use_cftime=True )
    ...
    for coord in FWI.coords:
        if coord == 'time': # can be /bnds/bounds
            out.coords[coord] = xr.concat( [init[coord].sel(time=t2040), FWI[coord]], dim='time' )
        else:
    ...
    for var in FWI.variables:
        if var == 'fwi':
            out['fwi'] = xr.concat( [init['fwi'].sel(time=t2040), FWI['fwi']], dim='time' )
        elif var in ['time_bnds', 'time_bounds']:
            out[var] = xr.concat( [init[var].sel(time=t2040).compute(), FWI[var].compute()], dim='time' )
