...
class tree_FWI_CMIP6:
    ...
    def edit_radius_historical(self, XX, YY, fact_rad):
        ...
        new_pos = lambda fact, start, end: (
            XX[start] + fact * (XX_cp[end] - XX_cp[start]),
            YY[start] + fact * (YY_cp[end] - YY_cp[start]),
        )
        XX[self.dico_mem["historical"]], YY[self.dico_mem["historical"]] = new_pos(
            fact_rad, self.dico_mem["CMIP6"], self.dico_mem["historical"]
        )
    ...
    def calculate_positions_nodes(
        ...
        layout,
        fact_rad=1,
        margin_angle=5 / 360 * 2 * np.pi,
        radius={"xps": 1, "esms": 1, "membs": 1},
        **args
    ):
        ...
        for i_xp, xp in enumerate(self.list_xps):
            ...
            self.labels.append(
                ...
                + str(
                    sum(
                        [
                            len(self.xp_avail_esms[xp][esm])
                            for esm in self.xp_avail_esms[xp]
                        ]
                    )
                )
                + ")"
        ...
        if layout == "personal":
            ...
            self.lay.append(
                self.func_coord(
                    origin=self.lay[0],
                    n_nodes=len(self.list_xps),
                    radius=radius["xps"],
                    available_angle=[0, 2 * np.pi],
                )
            )
        ...
        for i_xp, xp in enumerate(self.list_xps):
            for i_esm, esm in enumerate(self.xp_avail_esms[xp].keys()):
                if esm in self.xp_avail_esms[xp]:
                    ...
                    self.list_links.append(
                        (self.dico_mem[xp], self.dico_mem[xp + "/" + esm])
                    )
            ...
            if layout == "personal":
                ...
                self.lay.append(
                    self.func_coord(
                        origin=self.lay[1][i_xp, :],
                        n_nodes=int(len(self.xp_avail_esms[xp])),
                        radius=radius["esms"],
                        available_angle=range_angle,
                    )
                )
        ...
        for i_xp, xp in enumerate(self.list_xps):
            for i_esm, esm in enumerate(self.xp_avail_esms[xp].keys()):
                ...
                self.list_links.append(
                    (
                        self.dico_mem[xp + "/" + esm],
                        self.dico_mem[xp + "/" + esm + "/" + "memb"],
                    )
                )
                self.labels.append(str(len(self.xp_avail_esms[xp][esm])))
                if layout == "personal":
                    # adding coordinates on third level (members) of this second node
                    range_angle = self.find_range_angle(
                        self.lay[1 + 1 + i_xp], i_esm, margin_angle
                    )  # will have only one output from here
                    self.lay.append(
                        self.func_coord(
                            origin=self.lay[1 + i_xp + 1][i_esm, :],
                            n_nodes=1,
                            radius=radius["membs"],
                            available_angle=range_angle,
                        )
    ...
    def find_range_angle(self, coords, i_xp, margin_angle):
        ...
        # size_range
        size_range = [
            np.abs(max_angle - central_angle),
            np.abs(central_angle - min_angle),
        ]
        for i in [0, 1]:
    ...
    @staticmethod
    def func_coord(
        origin, n_nodes, radius, available_angle=[0, 2 * np.pi], weights=None
    ):
        ...
        # deduce angles
        angles = available_angle[0] + (
            available_angle[1] - available_angle[0]
        ) * np.cumsum(tmp_weights) / sum(tmp_weights)

        # give simply cartesian coordinates of equally spaced points in a given portion of an angle
        for i in range(n_nodes):
            out[i, :] = [
                origin[0] + radius * np.cos(angles[i]),
                origin[1] + radius * np.sin(angles[i]),
            ]
        return out
    ...
    def make_annotations(self, font_size, font_color="rgb(0,0,0)"):
        ...
        annotations = [
            dict(
                ...
                xref="x",
                yref="y",
                font=dict(color=font_color, size=font_size["CMIP6"]),
                textangle=0,
                showarrow=False,
            )
        ]
        ...
        for k in range(len(self.list_links)):
            ...
            annotations.append(
                dict(
                    ...
                    xref="x",
                    yref="y",
                    font=dict(color=font_color, size=ftsz),
                    textangle=ang,
                    showarrow=False,
                )
            )
...
class GWL:
    ...
    def __init__(self):
        ...
        self.dico_extensions = {"historical": "ssp245"} | {
            ss: "historical"
            for ss in [
                "ssp245",
                "ssp370",
        ...
        self.dico_periods = {"historical": (1850, 2014)} | {
            ss: (2015, 2100)
            for ss in [
                "ssp245",
                "ssp370",
    ...
    def prep_cmip6ng(self, files_cmip6ng, n_years, ref_period):
        ...
        with warnings.catch_warnings():
            ...
            self.DATA_cmip6ng = xr.open_mfdataset(
                self.files_cmip6ng,
                preprocess=self.func_preprocess_annual_spatial_average,
            )
    ...
    def func_preprocess_annual_spatial_average(data):
        ...
        data["tas"] = (
            data["tas"]
            * weights_area[np.newaxis, :, np.newaxis]
            / (data.lon.size * sum(weights_area))
        ).sum(("lat", "lon"))
        return data

    def concat_historical(self):
        tm = slice(
            self.dico_periods["historical"][0], self.dico_periods["historical"][1]
        )
        sc_nh = [scen for scen in self.DATA_cmip6ng.scen.values if scen != "historical"]
        for scen in sc_nh:
            self.DATA_cmip6ng["tas"].loc[
                {"scen": scen, "time": tm}
            ] = self.DATA_cmip6ng["tas"].sel(scen="historical", time=tm)
        self.DATA_cmip6ng = self.DATA_cmip6ng.sel(scen=sc_nh)

    def rolling_mean(self):
        if True:
            self.DATA_cmip6ng["tas"] = (
                self.DATA_cmip6ng["tas"]
                .rolling(time=self.n_years, center=True)
                .mean("time")
            )
        else:
            ...
            for scen in self.DATA_cmip6ng.scen.values:
                ...
                time_scen = slice(
                    self.dico_periods[scen][0], self.dico_periods[scen][1]
                )
                # merging historical and ssp245   OR    historical and scenario
                tmp = xr.merge(
                    [
                        self.DATA_cmip6ng["tas"]
                        .sel(scen=scen, time=time_scen)
                        .drop("scen"),
                        self.DATA_cmip6ng["tas"]
                        .sel(scen=ext, time=time_ext)
                        .drop("scen"),
                    ]
                )
                # rolling mean
                self.DATA_cmip6ng["tas"].loc[{"scen": scen, "time": time_scen}] = (
                    tmp["tas"]
                    .rolling(time=self.n_years, center=True)
                    .mean("time")
                    .sel(time=time_scen)
    ...
    def remove_ref_period(self):
        ...
        if True:
            self.DATA_cmip6ng["tas"] -= (
                self.DATA_cmip6ng["tas"]
                .sel(time=slice(self.ref_period[0], self.ref_period[1]))
                .mean("time")
            )
        else:
            self.DATA_cmip6ng["tas"] -= (
                self.DATA_cmip6ng["tas"]
                .sel(
                    time=slice(self.ref_period[0], self.ref_period[1]),
                    scen="historical",
                )
                .mean("time")
                .drop("scen")
    ...
    def find_position_GWLs(self, list_GWLs, option_common_set=False):
        ...
        self.DATA_cmip6ng["year_GWL"] = xr.DataArray(
            np.nan,
            coords={d: self.DATA_cmip6ng[d] for d in ["esm", "member", "scen", "GWL"]},
            dims=("esm", "member", "scen", "GWL"),
        )
        self.dico_ind_GWL = {GWL: [] for GWL in self.list_GWLs}
        for GWL in self.list_GWLs:
            sel = (self.DATA_cmip6ng["tas"] - GWL) > 0.0
            ...
            for i in range(len(ind[0])):
                ...
                for i_dd, dd in enumerate(self.DATA_cmip6ng["tas"].dims):
                    ...
                    if dd == "time":
                        ...
                        yr0 <= self.dico_periods["historical"][1]
                    ):  # due to the merge of historical
                        pos[
                            dd
                        ] = "historical"  # self.DATA_cmip6ng[dd].values[ ind[i_dd][i] ]#  'historical' # /!\: KEEPING THAT ONE, OTHERWISE CAUSING ISSUES, PARTICULARLY WITH option_common_set
                    else:
        ...
        if option_common_set:
            ...
            for pos in lst[self.list_GWLs[-1]]:
                test = True
                for GWL in self.list_GWLs:
                    if (pos not in lst[GWL]) and (
                        ["historical", pos[1], pos[2]] not in lst[GWL]
                    ):
                        test = False
                if test:
                    common.append(pos)
            ...
            for GWL in self.list_GWLs:
                ...
                for pos in self.dico_ind_GWL[GWL]:
                    if pos["time"] <= self.dico_periods["historical"][1]:
                        test = np.any(
                            [scen, pos["esm"], pos["member"]] in self.dico_ind_GWL[GWL]
                            for scen in list(self.DATA_cmip6ng.scen.values)
                            + ["historical"]
                        )
                    else:
    ...
    def apply_GWL(self, data_in, option_calc_on_GWL="mean"):
        ...
        data_out["maps_GWL"] = xr.DataArray(
            np.nan,
            coords={
                d: data_out[d] for d in ["esm", "member", "scen", "lat", "lon", "GWL"]
            },
            dims=("esm", "member", "scen", "lat", "lon", "GWL"),
        )

        for gwl in self.list_GWLs:
            for i_pos, pos in enumerate(self.dico_ind_GWL[gwl]):
                print(
                    "Selecting maps for GWL "
                    + str(gwl)
                    + "K: "
                    + str(i_pos + 1)
                    + "/"
                    + str(len(self.dico_ind_GWL[gwl])),
                    end="\r",
                )
                # identifying
                scen, esm, member, yr0 = (
                ...
                time_scen = np.arange(
                    self.dico_periods[scen][0], self.dico_periods[scen][1] + 1
                )

                # getting data
                if (yr0 - self.n_years / 2 < time_scen[0]) or (
                    yr0 + self.n_years / 2 - 1 > time_scen[-1]
                ):
                    # need an extension
                    ext = self.dico_extensions[scen]
                    time_ext = np.arange(
                        self.dico_periods[ext][0], self.dico_periods[ext][1] + 1
                    )

                    # identifying smaller periods to load
                    if (
                        yr0 - self.n_years / 2 < time_scen[0]
                    ):  # ssp scenario with part of the data to select BEFORE in historical
                    ...
                    tmp = xr.merge(
                        [
                            data_in.sel(
                                esm=esm, member=member, scen=scen, time=tt_scen
                            ).drop("scen"),
                            data_in.sel(
                                esm=esm, member=member, scen=ext, time=tt_ext
                            ).drop("scen"),
                        ]
                    )[data_in.name]
                    ...
                    tmp = data_in.sel(
                        ...
                        member=member,
                        scen=scen,
                        time=slice(yr0 - self.n_years / 2, yr0 + self.n_years / 2 - 1),
                    ).drop("scen")

                # average over time:
                if option_calc_on_GWL == "mean":
                    data_out["maps_GWL"].loc[
                        {"GWL": gwl, "esm": esm, "scen": scen, "member": member}
                    ] = tmp.mean(
                        "time"
                    )  # ('lat','lon')
                    ...
                    data_out["maps_GWL"].loc[
                        {"GWL": gwl, "esm": esm, "scen": scen, "member": member}
                    ] = tmp.quantile(q=lvl / 100, dim=("time")).drop(
                        "quantile"
                    )  # ('lat','lon')
                else:
...
class maps_mean_uncertainties:
    ...
    def eval_robust_certain(
        ...
        approach_ipcc,
        data_gamma,
        limit_certainty_members,
        limit_certainty_ESMs,
        limit_robustness_members,
        limit_robustness_ESMs,
        mask_map=None,
    ):
        # archiving
        ...
        self.where_runs = {
            val_dim: self.check_where_runs(maps_to_plot.loc[{self.dim_plot: val_dim}])
            for val_dim in maps_to_plot[self.dim_plot].values
        }

        # checking where the signs are robust
        self.robust_change = self.is_robust(
            limit_robustness_members, limit_robustness_ESMs
        )

        # checking where the changes are certain
        if approach_ipcc == "C":
            self.calc_gamma(data_in=data_gamma, mask_map=mask_map)
            self.certain = self.is_certain(
                limit_certainty_members, limit_certainty_ESMs
            )
        ...
        for esm in map_check.esm.values:
            for scen in map_check.scen.values:
                for member in map_check.member.values:
                    if (
                        np.isnan(vals00.sel(esm=esm, scen=scen, member=member).values)
                        == False
                    ):
                        if esm not in out:
                            out[esm] = {}
    ...
    def handle_test(test, where_runs, val_dim, limit_members, limit_ESMs):
        ...
        tot_numb = xr.DataArray(
            0, coords={"esm": test["esm"].sel(esm=esms)}, dims=("esm",)
        )
        for esm in esms:
    ...
    def is_certain(self, limit_certainty_members=0.66, limit_certainty_ESMs=0.66):
        certain = xr.DataArray(
            np.nan,
            coords={
                dd: self.maps_to_plot[dd].values for dd in [self.dim_plot, "lat", "lon"]
            },
            dims=(self.dim_plot, "lat", "lon"),
        )

        for val_dim in self.maps_to_plot[self.dim_plot].values:
            print(
                "Verifying where the signal is certain in "
                + self.dim_plot
                + ": "
                + str(val_dim),
                end="\r",
            ...
            test = (
                self.maps_to_plot.loc[{self.dim_plot: val_dim}] - self.map_ref
            ) > self.gamma
            certain.loc[{self.dim_plot: val_dim}] = self.handle_test(
                test,
                self.where_runs,
                val_dim,
                limit_certainty_members,
        ...
        print(
            "Verifying where the signal is certain in " + self.dim_plot + ": done.",
            end="\n",
        )
    ...
    def is_robust(self, limit_robustness_members=0.80, limit_robustness_ESMs=0.80):
        ...
        for val_dim in self.maps_to_plot[self.dim_plot].values:
            print(
                "Verifying where the changes are robust in "
                + self.dim_plot
                + ": "
                + str(val_dim),
                end="\r",
            ...
            robustness.loc[{self.dim_plot: val_dim}] = self.handle_test(
                test,
                self.where_runs,
                val_dim,
                limit_robustness_members,
    ...
    def calc_sigma_1yr(self, data_in):
        print(
            "Calculating standard deviation over "
            + str(self.period_calc_gamma[0])
            + "-"
            + str(self.period_calc_gamma[1])
        )

        # preparing output variable
        self.sigma_1yr = xr.DataArray(
            np.nan,
            coords={
                "esm": data_in.esm.values,
                "member": data_in.member.values,
                "lat": data_in.lat.values,
                "lon": data_in.lon.values,
            },
            dims=("esm", "member", "lat", "lon"),
        )

        # preparing linear detrend
        x = np.vstack(
            [
                np.ones(self.period_calc_gamma[1] - self.period_calc_gamma[0] + 1),
                data_in.time.values,
            ]
        ).T
        ...
        for esm in data_in.esm:
            for member in data_in.member:
                if np.any(np.isnan(y_all.sel(esm=esm, member=member).values)) == False:
                    y = (
                        y_all.sel(esm=esm, member=member)
                    ...
                    self.sigma_1yr.loc[{"esm": esm, "member": member}] = np.std(
                        y - y_fit, axis=0
                    )  # ('lat','lon')
    ...
    def plot(
        ...
        unit_dim,
        label_row,
        plot_diff_GWLs=False,
        vmin=None,
        vmax=None,
        levels_cmap=9,
        fontsize={"colorbar": 14, "title": 14, "legend": 12, "label_row": 14},
        density_visual_code={"x": 2, "\\": 2, "/": 2},
        margin_colorbar_pct=1,
        legend_val_dim=None,
    ):
        # overall preparation
        counter_letter = (
            ind_row * len(self.maps_to_plot[self.dim_plot]) + (ind_row != 0) * 1
        )

        # preparation
        lon_mesh, lat_mesh = np.meshgrid(
            self.maps_to_plot.lon.values, self.maps_to_plot.lat.values
        )
        self.data_plot = {}
        self.esms_plot = {}
        self.tot_numb_plot = {}
        ...
        (
            self.data_plot["ref"],
            self.esms_plot["ref"],
            self.tot_numb_plot["ref"],
        ) = self.average_map(
            dat_in=self.map_ref,
            val_dim=self.maps_to_plot[self.dim_plot].values[0],
            option_multiple_scens=False,
        )
        for val_dim in self.maps_to_plot[self.dim_plot].values:
            # preparing data, computing ONLY required ones:
            (
                self.data_plot[val_dim],
                self.esms_plot[val_dim],
                self.tot_numb_plot[val_dim],
            ) = self.average_map(
                dat_in=self.maps_to_plot.loc[{self.dim_plot: val_dim}],
                val_dim=val_dim,
                option_multiple_scens=True,
            )

        # preparing figure
        ax.coastlines()
        cmaps = self.prepare_colormaps(
            vmin, vmax, levels_cmap, margin_colorbar_pct, plot_diff_GWLs
        )
        ...
        if plot_diff_GWLs:
            cbar = plt.colorbar(
                pmesh, fraction=0.05, pad=0.04, orientation="vertical"
            )  # fraction=0.0235, default 0.15
        ...
        if do_title:
            plt.title(
                str(self.reference_period[0])
                + "-"
                + str(self.reference_period[1])
                + "\n("
                + str(self.tot_numb_plot["ref"])
                + " runs over "
                + str(len(self.esms_plot["ref"]))
                + " ESMs)",
                size=fontsize["title"],
            )
        ax.text(
            ...
            0.55,
            s=label_row,
            fontdict={"size": fontsize["label_row"]},
            color="k",
            va="bottom",
            ha="center",
            rotation="vertical",
        ...
        plt.text(
            ...
            fontdict={"size": 0.8 * fontsize["title"]},
        )
        counter_letter += 1
        ...
        for i_dim, val_dim in enumerate(self.maps_to_plot[self.dim_plot].values):
            ...
            if plot_diff_GWLs == False:
                ...
                pmesh = ax.pcolormesh(
                    lon_mesh,
                    lat_mesh,
                    self.data_plot[val_dim] - self.data_plot["ref"],
                    transform=ccrs.PlateCarree(),
                    rasterized=True,
                    cmap=cmaps["cmap_diff"],
                    vmin=cmaps["vmin_diff"],
                    vmax=cmaps["vmax_diff"],
                )
            ...
            if self.approach_ipcc == "B":
                ...
                ax.contourf(
                    ...
                    lat_mesh,
                    xr.where(
                        (self.robust_change == 0).loc[{self.dim_plot: val_dim}],
                        1,
                        np.nan,
                    ),
                    transform=ccrs.PlateCarree(),
                    colors="none",
                    hatches=[density_visual_code["/"] * "/"],
                )
                # ghost plot for legend
                plt.fill_between(
                    lon_mesh[0, :],
                ...
                ax.contourf(
                    ...
                    lat_mesh,
                    xr.where(
                        (self.certain == 0).loc[{self.dim_plot: val_dim}], 1, np.nan
                    ),
                    transform=ccrs.PlateCarree(),
                    colors="none",
                    hatches=[density_visual_code["\\"] * "\\"],
                ...
                ax.contourf(
                    ...
                    lat_mesh,
                    xr.where(
                        ((self.certain == 1) * (self.robust_change == 0)).loc[
                            {self.dim_plot: val_dim}
                        ],
                        1,
                        np.nan,
                    ),
                    transform=ccrs.PlateCarree(),
                    colors="none",
                    hatches=[density_visual_code["x"] * "x"],
                )
                # ghost plot for legend
                plt.fill_between(
                    lon_mesh[0, :],
            ...
            if do_title:
                plt.title(
                    ...
                    + unit_dim
                    + "\n("
                    + str(self.tot_numb_plot[val_dim])
                    + " runs over "
                    + str(len(self.esms_plot[val_dim]))
                    + " ESMs)",
                    size=fontsize["title"],
                )
            if (legend_val_dim is not None) and (legend_val_dim == val_dim):
                plt.legend(
                    loc="center",
                    bbox_to_anchor=[0.5, -0.11],
                    prop={"size": fontsize["legend"]},
                    ncol=3,
                )
            plt.text(
                x=ax.get_xlim()[0] + 0.05 * (ax.get_xlim()[1] - ax.get_xlim()[0]),
        ...
        for esm in esms:
            ...
            if option_multiple_scens:
                ...
                for scen in scens:
                    ...
                    tmp.loc[{"scen": scen}] = dat_in.loc[
                        {
                            "esm": esm,
                            "scen": scen,
                            "member": self.where_runs[val_dim][esm][scen],
                        }
                    ].mean("member")
                ...
                members = list(
                    set(
                        [
                            memb
                            for scen in self.where_runs[val_dim][esm].keys()
                            for memb in self.where_runs[val_dim][esm][scen]
                        ]
                    )
                )
                TMP.loc[{"esm": esm}] = dat_in.loc[
                    {"esm": esm, "member": members}
                ].mean(
                    "member"
                )  # no scen dimension, because in historical
    ...
    def prepare_colormaps(
        self, vmin, vmax, levels_cmap, margin_colorbar_pct, plot_diff_GWLs
    ):
        cmap = cm.get_cmap("Reds", levels_cmap)
        if plot_diff_GWLs == False:
            if vmin is None:
                vmin = min(
                    [np.nanpercentile(self.data_plot["ref"], margin_colorbar_pct)]
                    + [
                        np.nanpercentile(self.data_plot[val_dim], margin_colorbar_pct)
                        for val_dim in self.maps_to_plot[self.dim_plot].values
                    ]
                )
            if vmax is None:
                vmax = max(
                    [np.nanpercentile(self.data_plot["ref"], 100 - margin_colorbar_pct)]
                    + [
                        np.nanpercentile(
                            self.data_plot[val_dim], 100 - margin_colorbar_pct
                        )
                        for val_dim in self.maps_to_plot[self.dim_plot].values
                    ]
                )
            ...
            if vmax is None:
                vmax = np.nanpercentile(
                    self.data_plot["ref"], 100 - margin_colorbar_pct
                )
                vmax_diff = max(
                    [
                        np.nanpercentile(
                            self.data_plot[val_dim] - self.data_plot["ref"],
                            100 - margin_colorbar_pct,
                        )
                        for val_dim in self.maps_to_plot[self.dim_plot].values
                    ]
                )
            ...
            order_mag = max(
                [
                    np.int32(np.log10(np.abs(vmin_diff))),
                    np.int32(np.log10(np.abs(vmax_diff))),
                ]
            )
            ...
            if vmin_diff < 0:
                size_bins = np.round(
                    (vmax_diff - vmin_diff) / levels_cmap, 1 - order_mag
                )
            ...
            cmap_diff = plcol.LinearSegmentedColormap.from_list(
                "new_colormap",
                list(zip(map(norm_diff, cvals), colors)),
                levels_cmap + 1,
            )
...
def plot_maps_timeseries(
    ...
    for yr in list_years:
        if yr - int(window_average / 2) < DATA.time.values[0]:
            years_bnds[yr] = (
                DATA.time.values[0],
                DATA.time.values[0] + window_average - 1,
            )
        elif yr + int(window_average / 2) > DATA.time.values[-1]:
            years_bnds[yr] = (
                DATA.time.values[-1] - window_average + 1,
                DATA.time.values[-1],
            )
        else:
            years_bnds[yr] = (
                yr - int(window_average / 2),
                yr + int(window_average / 2) - 1,
            )
    ...
    for i_yr, year in enumerate(list_years):
        ...
        TMP = (
            DATA[indic]
            .sel(scen=scen, time=slice(years_bnds[year][0], years_bnds[year][1]))
            .loc[{axis_ref: value_ref}]
            .mean("time")
        )
    ...
    for i_band, band in enumerate(lat_bands):
        ...
        TMP = weights[i_lat_min:i_lat_max, :] * DATA[indic].loc[
            {axis_ref: value_ref}
        ].isel(lat=slice(i_lat_min, i_lat_max))
        TMP = TMP.sum(("lat", "lon")) / np.sum(weights[i_lat_min:i_lat_max, :])
        TMP = xr.concat(
            [
                TMP.sel(scen=xps[0], time=slice(1850, 2014)),
                TMP.sel(scen=xps[1], time=slice(2015, 2100)),
            ],
            dim="time",
        )
    ...
    spec = gridspec.GridSpec(
        ncols=len(list_years),
        nrows=2 + len(lat_bands),
        figure=fig,
        height_ratios=[1, 1] + [1 / 2] * len(lat_bands),
        left=0.05,
        right=0.95,
        bottom=0.025,
        top=0.975,
    ...
    plt.suptitle(
        dico_indics[indic] + " of " + value_ref + " accross " + txt_mode,
        size=size_text,
        y=0.999,
    )
    ...
    for i_yr, year in enumerate(list_years):
        ...
        plt.title(
            str(years_bnds[year][0])
            + "-"
            + str(years_bnds[year][1])
            + " ("
            + scen
            + ")",
            size=size_text,
    ...
    for i_band, band in enumerate(lat_bands):
        ...
        plt.plot(
            TMP.time,
            np.nan
            * dic_TMP_lat[band].loc[{axis_comparison: TMP[axis_comparison][0]}].values,
            lw=1,
            color="gray",
            label=txt_mode,
            zorder=1,
...
def matching_scenarios(list_files):
    ...
    for file_W in list_files:
        var, _, esm, xp, memb, grid = str.split(
            file_W[: -len(".nc")], "_"
        )  # no need for "time_res"
...
def func_load_sensitivity(
    ...
    for value_axis in options:
        ...
        cfg = configuration_FWI_CMIP6(
            prop_calc_FWI_ref["type_variables"],
            prop_calc_FWI_ref["adjust_DryingFactor"],
            prop_calc_FWI_ref["adjust_DayLength"],
            prop_calc_FWI_ref["adjust_overwinterDC"],
            limits_on_data={
                "xps": limits["scen"],
                "members": limits["memb"],
                "esms": limits["esm"],
            },
            path_cmip6=prop_calc_FWI_ref["path_cmip6"],
            path_save=prop_calc_FWI_ref["path_save"],
            overwrite_path_saveFWI=None,
            option_overwrite=False,
            overwrite_before="9999-01-01T00:00:00",
            option_load_available=True,
            option_calc_size=False,
            option_mask_land=False,
            option_full_outputs=prop_calc_FWI_ref["option_full_outputs"],
        )
        ...
        for file_W in total_lst_files:
            ...
            if (
                (limits["esm"] is None or det[2] in limits["esm"])
                and (limits["scen"] is None or det[3] in limits["scen"])
                and (limits["memb"] is None or det[4] in limits["memb"])
                and (
                    prop_calc_FWI_ref["option_full_outputs"]
                    and ("full-outputs" in file_W)
                    or (prop_calc_FWI_ref["option_full_outputs"] == False)
                )
            ):
        ...
        if len(limit_list_files) == 0:
            ...
            for file_W in limit_list_files:
                ...
                if data.experiment_id == "historical":
                    for _var in subset_vars:
                        results[value_axis]["map_" + _var] = (
                            data[_var].sel(time=date_maps).compute()
                        )

                # timeseries
                for _var in subset_vars:
                    results[value_axis][
                        "time_map_" + _var + "_" + data.experiment_id
                    ] = (
                        data[_var]
                        .sel(
                            time=slice(
                                period_timeseries[data.experiment_id][0],
                                period_timeseries[data.experiment_id][1],
                            )
                        )
                        .compute()
                    )
            ...
            if mask_FWI is not None:
                for _var in results[value_axis].keys():
                    results[value_axis][_var] = xr.where(
                        mask_FWI, results[value_axis][_var], np.nan
                    )
    return results
...
def get_spatial_info(
    path_cmip6, esm, scen, memb, grid, list_spat_vars=["sftlf", "areacella"]
):
    out = xr.Dataset()
    ...
    for VAR in list_spat_vars:
        ...
        path_tmp = os.path.join(
            ...
            memb,
            grid,
            VAR + "_fx_" + esm + "_" + scen + "_" + memb + "_" + grid + ".nc",
        )
